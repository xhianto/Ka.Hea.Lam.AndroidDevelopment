...
class DataBaseService(context: Context?) :
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val createTableStatement =
            "CREATE TABLE " + USER_TABLE + " (" + COLUMN_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, '" + COLUMN_EMAILADDRESS + "' TEXT NOT NULL UNIQUE, '" + COLUMN_USERNAME + "' TEXT, " + COLUMN_EASYWIN + " INT, " + COLUMN_EASYDRAW + " INT, " + COLUMN_EASYLOSE + " INT, " + COLUMN_HARDWIN + " INT, " + COLUMN_HARDDRAW + " INT, " + COLUMN_HARDLOSE + " INT, " + COLUMN_FRIENDWIN + " INT, " + COLUMN_FRIENDDRAW + " INT, " + COLUMN_FRIENDLOSE + " INT)"
    ...
    fun getUserByEmail(email: String): User {
        ...
        if (cursor.moveToFirst()) {
            ...
            user.easyWin = cursor.getInt(3)
            user.easyDraw = cursor.getInt(4)
            user.easyLose = cursor.getInt(5)
            user.hardWin = cursor.getInt(6)
            user.hardDraw = cursor.getInt(7)
            user.hardLose = cursor.getInt(8)
            user.friendWin = cursor.getInt(9)
            user.friendDraw = cursor.getInt(10)
            user.friendLose = cursor.getInt(11)
        }
        cursor.close()
        db.close()
        return user
